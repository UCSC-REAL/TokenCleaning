...
bash_src/finetune.sh "$model_name_or_path" "$finetune_model_tag" "$train_data_tag" "$max_seq_length" "$BATCH_SIZE_PER_GPU" "$NUM_GPUS" "$base_model" "$TOTAL_BATCH_SIZE" "$cluster_root_path" "$data_prop" "$main_process_port" 


# nohup bash run_reference_model.sh > zzz_reference_model_selected.log &

...
# Train_DATASET_LIST=("filtered-cured-50k-active-split-global-curve-positive-fixed-base-loss-using-warmup-full-data")
...
do
    ...
    for data_prop in ${data_prop_list[@]}; do
        ...
        # start_subset_idx=0
        # for (( idx=$start_subset_idx; idx<${#train_data_tag_list[@]}; idx++ )); do
...
# bash run_explore_active_split_impact_using_warmup-reference-model.sh > zzz_filtered-cured-50k-active-split-global-curve-positive-fixed-base-loss-using-warmup-full-data.log 2>&1
...
def convert_sharegpt_data(
    data_dir, output_dir, data_file="sharegpt_html_cleaned_and_split_2048.json", num_examples=100000
):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    with open(os.path.join(data_dir, data_file), "r") as fin:
        examples.extend(json.load(fin))
    if num_examples:
        examples = random.sample(examples, k=min(num_examples, len(examples)))
...
def convert_wizardlm_data(data_dir, output_dir, num_examples=100000):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    ...
    if os.path.exists(os.path.join(data_dir, "WizardLM_evol_instruct_V2_143k.json")):
        ...
        df = pd.read_parquet(os.path.join(data_dir, "train-00000-of-00001.parquet"))
